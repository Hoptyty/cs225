...
void CommonWords::init_file_word_maps(const vector<string>& filenames) 
{
	...
	for(size_t j = 0; j<words.size(); j++) {
		auto lookup=memo.find(words[j]);
		if(lookup==memo.end())
			memo[words[j]]=1;
		else
			lookup->second++;
	}
	file_word_maps.push_back(memo);
    }
}
...
void CommonWords::init_common() 
{
		...
		for(map<string,unsigned int>::iterator j=file_word_maps[i].begin();j!=file_word_maps[i].end();j++){
			auto lookup=common.find(j->first);
