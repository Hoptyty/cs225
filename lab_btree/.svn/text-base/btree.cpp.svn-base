...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	for(auto i=mid_elem_itr; i!= child->elements.end(); i++)
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
	auto inPos = subroot->elements.begin()+first_larger_idx;
	bool omg;
	if(first_larger_idx<subroot->elements.size())
		 omg = (subroot->elements[first_larger_idx]==pair.key);
	else
		 omg = false;
	if(omg)
		return;
		...
		if(subroot->is_leaf)
			subroot->elements.insert(inPos, pair);
		else insert(subroot->children[first_larger_idx],pair);
	}
	if(!subroot->is_leaf)
