...
//    	return myInsertion(elements, val, 0, len);
//}


template <class T, class C>
size_t myInsertion(const std::vector< T >& elements, const C& val, size_t x, size_t y)
{
	size_t mid = (x + y) / 2;
	if(elements[mid]==val)
		return mid;
	if(x>=y)
		return y;
	if(elements[mid] < val)
		return myInsertion(elements, val, mid+1, y);
	else
		return myInsertion(elements, val, x, mid);
}
