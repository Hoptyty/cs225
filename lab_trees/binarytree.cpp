...
{	
	vector<T> myVec;
	return isOrdered(root,myVec);;
	// your code here
}


template <typename T>
bool BinaryTree<T>::isOrdered(Node* subRoot, vector<T> & myStack ) const
{	
	bool retval=true;
	bool retval2=true;
	if(subRoot==NULL)
		return true;
	retval=isOrdered(subRoot->left,myStack);
	if(!myStack.empty())
		if(myStack.back()>subRoot->elem)
			return false;
	myStack.push_back(subRoot->elem);
	retval2=isOrdered(subRoot->right,myStack); 				
...
{	
   	vector<T> myVec; 
	printPaths(root,myVec);// your code here
}
/**
...
template <typename T>
void BinaryTree<T>::printPaths(Node* subRoot, vector<T>  myVec) const
{
...
{	
	vector<T> myVec;
    	return sumDistances(root,myVec);// your code here
}

template <typename T>
int BinaryTree<T>::sumDistances( Node* subRoot, vector<T> myVec) const
{	
	int sum = 0;
	if(subRoot == NULL)	
        return 0;
	myVec.push_back( subRoot->elem );
	return (int)myVec.size() - 1 + sumDistances(subRoot->left, myVec) + sumDistances(subRoot->right, myVec);	
}
