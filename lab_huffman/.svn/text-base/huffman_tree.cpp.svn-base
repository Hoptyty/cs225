...
HuffmanTree::TreeNode * HuffmanTree::removeSmallest( 
	...
	smallest=mergeQueue.front();// your code!
	mergeQueue.pop();
	}
	return smallest;
...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	...
	TreeNode * myLeft =removeSmallest(singleQueue,mergeQueue);
	TreeNode * myRight=removeSmallest(singleQueue,mergeQueue);
	TreeNode * myNode = new TreeNode(myLeft->freq.getFrequency()+myRight->freq.getFrequency());
	myNode->left=myLeft;
	myNode->right=myRight;
	mergeQueue.push(myNode);
	}
...
HuffmanTree::TreeNode * HuffmanTree::readTree( BinaryFileReader & bfile ) {
	...
	if(!bfile.hasBits())	return NULL; // replaceme!
	if(bfile.getNextBit()==1) {
		TreeNode * myTreeNode= new TreeNode(Frequency(bfile.getNextByte(),0));
		return myTreeNode;
		}
	else {
		TreeNode * myTreeNode= new TreeNode(0);
		
		myTreeNode->left=readTree(bfile);
		
		myTreeNode->right=readTree(bfile);
		return myTreeNode;
	}
