...
class KDTree
{
    ...
    private:
         ...
         */
	void buildTree(vector< Point<Dim> > & points, int x, int y, int currDim);
    int ctrHelper(vector< Point<Dim> > & points, int x, int y, int mid, int currDim);
	int helpHelper(vector<Point<Dim>> & points, int x, int y, int n, int currDim);
    int findLeaf(const Point<Dim> & query, int x, int y, int currDim) const;
    int dist(const Point<Dim> & query, const Point<Dim> & myPoint) const;
};
