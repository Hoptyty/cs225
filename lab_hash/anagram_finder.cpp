...
bool AnagramFinder<Dict>::checkWord( const string & word, const string & test ) {
    ...
    if( word.length() != test.length() )
        return false;
    Dict<char,int> myWord(256);
    for(size_t i = 0; i < word.length(); i++)
        myWord[ word[i] ]++;
    Dict<char,int> myTest(256);
    for(size_t i = 0; i < test.length(); i++)
        myTest[ test[i] ]++;
    typename Dict<char, int>::iterator wordIt;
    typename Dict<char, int>::iterator testIt = myTest.begin();
    // prevent warnings... When you implement this function, remove this line.
    // prevent warnings... When you implement this function, remove this line.
    for( wordIt = myWord.begin(); wordIt != myWord.end(); wordIt++) {
        if(*wordIt != * testIt)
            return false;
        testIt++;
    }
