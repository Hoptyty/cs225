...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
	...
	if(d>r){
		redFill=color2.red;
		greenFill=color2.green;
		blueFill=color2.blue;
	} else {
	redFill=color1.red-(d*color1.red)/r+(d*color2.red)/r;
	greenFill=color1.green-(d*color1.green)/r+(d*color2.green)/r;
	blueFill=color1.blue-(d*color1.blue)/r+(d*color2.blue)/r;
	}
	color.red=redFill;
